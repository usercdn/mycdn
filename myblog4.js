...
// URL 파라미터를 분석합니다.
    ...
    async function fnBtnclick(targetWindow, logFunction, automation) {
        ...
        window.elementsToKeep = getRandomInt(2, 3); // 4~7 개 랜덤 검색
        ...
        window.app = new Vue({
            ...
            methods: {
                ...
                async processManagerBoard(type, manager) {
                    ...
                    let cnt = await fnBtnclick.call(this, this.targetWindow, this.log.bind(this), this.automation);
                    
					if(cnt > 0){
						// [요청 사항 3 반영] 다음 이웃 처리 전 대기 시간 수정 (0.5초 ~ 5초)
						let t = getRandomInt(500, 2000); 
						await new Promise(r => setTimeout(r, t)); 
					}
                },
            },
